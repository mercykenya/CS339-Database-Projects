...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        int card = 1;
        // DONE
        // Initialize the cardinality with a default value

        if (joinOp == Predicate.Op.EQUALS) {
            // If the join predicate is an equality condition

            if (t1pkey && !t2pkey) {
                // If table 1 is a primary-key table and table 2 is not
                card = card2; // Set the cardinality to the cardinality of table 2
            } else if (t2pkey && !t1pkey) {
                // If table 2 is a primary-key table and table 1 is not
                card = card1; // Set the cardinality to the cardinality of table 1
            } else {
                // If both tables are primary-key tables or neither is
                // Return the maximum cardinality of the two tables
                card = card1 > card2 ? card1 : card2;
         }
        } else {
            // For join predicates other than equality

            // Apply a default cardinality estimation strategy
            // Multiply the cardinalities of the two tables by a factor of 0.3
            card = (int) (0.3 * card1 * card2);
        }

        // Make sure the estimated cardinality is not negative or zero
    ...
    public List<LogicalJoinNode> orderJoins(
        ...
        // Create a set of all join nodes
        Set<LogicalJoinNode> joinSet = new HashSet<>(joins);
        joinSet.addAll(joins);
        
        // Enumerate all subsets of join nodes
        Set<Set<LogicalJoinNode>> j = enumerateSubsets(joins, 1);
        
        // Create a cache for storing optimal join plans
        PlanCache optjoin = new PlanCache();

        // Iterate over different sizes of subsets
        int sizeOfj = j.size();
        for (int i = 1; i <= sizeOfj; i++) {
            // Get subsets of join nodes of length i
            Set<Set<LogicalJoinNode>> lenISubsetOfj = enumerateSubsets(joins, i);

            // Consider each subset and find the best plan
            for (Set<LogicalJoinNode> s : lenISubsetOfj) {
                double bestCostSoFar = Double.MAX_VALUE;
                int bestCardSoFar = Integer.MAX_VALUE;

                // Initialize the best plan so far as an empty list
                List<LogicalJoinNode> bestPlanSoFar = new ArrayList<>();

                // Iterate over each join node in the current subset
                for (LogicalJoinNode s1 : s) {
                    // Compute the cost and cardinality of the subplan
                    CostCard costCard = computeCostAndCardOfSubplan(stats, filterSelectivities, s1, s, Double.MAX_VALUE, optjoin);

                    // Update the best plan if the current subplan is better
                ...
                // Add the best plan to the cache
                optjoin.addPlan(s, bestCostSoFar, bestCardSoFar, bestPlanSoFar);
            }
        }

        // Retrieve the best join order from the cache
        List<LogicalJoinNode> bestJoinOrder = new ArrayList<>(optjoin.getOrder(joinSet));
        
        // Return the best join order as a list
        return new ArrayList<>(bestJoinOrder);
